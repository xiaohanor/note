...
你可以在 [Unreal's docs](https://docs.unrealengine.com/5.3/en-US/API/Runtime/Core/Containers/TSortedMap/)中了解更多信息。
...
你可以通过 [Joel Olawanle](https://www.freecodecamp.org/news/big-o-cheat-sheet-time-complexity-chart/) 了解更多关于时间复杂度的信息。
...
当计算机读取内存时，它从 RAM（随机存取存储器）中读取。因此，它的名称是“随机访问内存”。

使用 `TArray` 时，内存分配是连续的。这意味着 `TArray` 会请求一段较大的连续空间来存放其内存块。然而，如果 `TArray` 在某个位置找不到或无法适应（因为 `TArray` 可以增长和缩小），它需要重新分配并找到一个新的位置。这会让添加或删除元素变得较为昂贵。

但 `TArray` 拥有链表所缺乏的优势：缓存命中。缓存命中是指 CPU 可以将数组数据预加载到 CPU 缓存中。由于数组存储在连续的内存中，CPU 能够高效预取，而无需在内存位置之间来回跳转。

对于链表来说，没有连续的内存布局。这意味着 CPU 需要在不同位置查找每个节点，不利于缓存预取，从而更容易发生“缓存未命中”。

参考：
- [SimonDev 关于缓存未命中/命中的视频](https://www.youtube.com/watch?v=247cXLkYt2M)
- [维基百科：更多关于 CPU 缓存的信息](https://en.wikipedia.org/wiki/Cache_(computing))

 **内存空间**

- `TList` 每个节点占用更多的内存空间，因为每个节点都需要跟踪列表中的下一个节点。这个“下一个”变量是一个指针：在 32 位计算机上占用 4 个字节，在 64 位计算机上占用 8 个字节。
- 如果你只使用基本数据类型（如 `int`、`float`、`double`、`bool` 或 `char`），通常可以直接使用 `TArray`。虽然 `TArray` 存在一些管理开销，但这个开销相对很小。

 **没有辅助函数**

- `TList` 仅提供数据元素和“下一个节点”的指针，不提供在列表中添加或删除节点的辅助函数。
- 如果你希望拥有这些辅助函数，可以考虑使用 `TLinkedList`。

 **只能向前遍历**

- 使用 `TList` 时，你只能向前移动，因为节点中没有指向前一个节点的指针；也就是说，无法向后遍历。
- 如果需要双向遍历，可以使用 `TDoubleLinkedList`。
...
您可以在[Unreal's docs](https://docs.unrealengine.com/5.3/en-US/API/Runtime/Core/Containers/TList/)中了解更多信息。
...
你可以在 [Unreal's docs](https://docs.unrealengine.com/5.3/en-US/API/Runtime/Core/Containers/TLinkedList/)中了解更多信息。
...
你可以在 [Unreal's docs](https://docs.unrealengine.com/5.3/en-US/API/Runtime/Core/Containers/TDoubleLinkedList/)中了解更多信息。
...
你可以在 Unreal's docs](https://docs.unrealengine.com/5.3/en-US/API/Runtime/Core/Containers/TQueue/)中了解更多信息。
...
您可以从 [Unreal's docs](https://docs.unrealengine.com/5.3/en-US/API/Runtime/Core/Containers/TArrayView/)中了解更多信息。
...
你可以在 [Unreal's docs](https://docs.unrealengine.com/5.3/en-US/API/Runtime/Core/Containers/TStringView/)中了解更多信息。
...
你可以在 [Unreal's docs](https://docs.unrealengine.com/5.3/en-US/API/Runtime/Core/Containers/TStringBuilderBase/)中了解更多信息。
...
你可以在[Unreal's docs](https://docs.unrealengine.com/5.3/en-US/API/Runtime/Core/Containers/TEnumAsByte/)中了解更多信息。
...
你可以在 [references in C++ from Low Level Learning](https://www.youtube.com/watch?v=wro8Bb6JnwU) 上观看关于 C++中引用的视频。
...
在引用中，你只能分配一次，并且在整个代码中不能更改。例如，你可以直接引用传递到函数中的参数。然后可以在函数内部修改这个参数，类似于 C#中的[输出](https://learn.microsoft.com/en-us/dotnet/csharp/language-reference/keywords/out-parameter-modifier)参数的工作方式 [6](#user-content-fn-12) 。
...
最后，我们来看指针。本节将介绍原始指针和智能指针。如果你对指针一无所知，强烈建议观看 [Cherno 关于指针](https://www.youtube.com/watch?v=DTxHyVn0ODg)的视频。
...
你可以在 [raw pointers from Microsoft Learn](https://learn.microsoft.com/en-us/cpp/cpp/raw-pointers?view=msvc-170) 上了解更多关于原始指针的信息。
...
你可以在[他们的文档](https://docs.unrealengine.com/5.2/en-US/smart-pointers-in-unreal-engine/)中了解更多关于 Unreal Smart Pointer Library 的信息。
...
在 C++中，关键字是编译器具有特殊含义的[保留字](https://en.wikipedia.org/wiki/Reserved_word)。关键字不能用作代码中变量、函数或其他标识符的名称。
...
这里有一个视频，由 [Cazz](https://www.youtube.com/watch?v=KBny6MZJR64) 解释 C++ 中常量关键字的作用。
...
*   `new` - 为对象分配内存（在[堆内存](https://en.wikipedia.org/wiki/Memory_management#HEAP)上）并调用其构造函数。
...
您可以从[Assertions from the docs](https://docs.unrealengine.com/5.1/en-US/asserts-in-unreal-engine/)中了解更多关于断言的信息。

您也可以从 [Sneaky Kitty Game Dev](https://www.youtube.com/watch?v=zGeJgI2xiT4) 观看关于它的视频。
...
你可以在[文档](https://docs.unrealengine.com/5.2/en-US/umg-ui-designer-for-unreal-engine/)中了解更多信息。

还有一个关于 Lively Geek 制作的 [UMG Widget 与 C++](https://www.youtube.com/watch?v=T7v3UnL6PNU)的视频。
...
您可以从[BenUI 的文章](https://benui.ca/unreal/ui-bindwidget/)中了解更多关于使用 C++绑定组件的信息。

### UI 缓动库

BenUI 还创建了一个免费的实用插件，帮助您在 C++中动画化 UMG。插件可以从 github [9](#user-content-fn-5) 安装。[仓库链接](https://github.com/benui-dev/UE-BUITween)。
...
您可以在[此处](https://dev.epicgames.com/documentation/en-us/unreal-engine/unreal-engine-modules)了解更多关于 Unreal Engine 模块的信息！
...
你可以在虚幻引擎的文档中了解更多关于[游戏玩法计时器](https://docs.unrealengine.com/5.3/en-US/gameplay-timers-in-unreal-engine/)的信息。
...
您可以在[虚幻引擎的博客](https://www.unrealengine.com/es-ES/tech-blog/using-gameplay-tags-to-label-and-organize-your-content-in-ue4)上了解更多关于 Gameplay 标签的信息。
...
你可以从[Robert Nystrom](https://gameprogrammingpatterns.com/game-loop.html)那里了解更多关于游戏循环的信息。

Ayliroé 撰写了一份关于 Unreal 的多线程和异步任务系统的精彩文档，你可以在 [Google Docs](https://docs.google.com/document/d/1uw9Dfui5ZepSrBpMc1DrxFOeRFnDu8ubzFse8Mr_s7E/) 或[Forum Post](https://forums.unrealengine.com/t/multithreading-and-performance-in-unreal/1216417/1)中阅读。
...
为了让您的游戏准备好支持多线程，您也需要改变思维方式。当将代码拆分到多个线程时可能会产生[竞态条件](https://en.wikipedia.org/wiki/Race_condition)，即两个操作同时发生，并竞争哪个将首先执行。这可能导致不稳定并引发错误。

您可以从 [Multithreading from Vulkan Guide](https://vkguide.dev/docs/extra-chapter/multithreading/) 中了解更多关于多线程的信息。

您也可以观看 [Chris Kanich about deadlocks](https://www.youtube.com/watch?v=oEbXlSH8hyE) 关于死锁的视频。

如果你想在蓝图内部创建多线程且只需最少的 C++代码，那么这里有一个[来自 Ispheria 项目的视频](https://www.youtube.com/watch?v=0Yyh3oQgonI)。
...
[TaskGraph](https://docs.unrealengine.com/5.0/en-US/tasks-systems-in-unreal-engine/)，是一个试图在多个现有线程之间平衡工作负载的工作管理器。这非常适合发送小操作包，因为它将线程管理的复杂性抽象化，还支持定义任务之间的依赖关系。
...
如果你不了解 lambda，那么强烈建议你阅读这[section](https://github.com/MrRobinOfficial/Guide-UnrealEngine/#-lambda)的介绍。

#### ParallelFor

`AsyncTask` 的一个[更高级版本](https://isaratech.com/ue4-improving-speed-with-parallelfor/)，它将一个 for 循环拆分成多个在 TaskGraph 中运行的 Tasks。
...
操作内部的顺序或线程安全性没有保证，因此你可能需要使用互斥锁或原子操作与它一起使用。MSVC 有[一个类似](https://learn.microsoft.com/en-us/cpp/preprocessor/loop?view=msvc-170)的#pragma loop(hint\_parallel(n))。实际上，你的循环内容必须足够重要才能真正从这种方法中受益。
...
如前所述，Ayliroé撰写了关于 Unreal 多线程和异步任务系统的精彩文档，你可以从 [Google Docs](https://docs.google.com/document/d/1uw9Dfui5ZepSrBpMc1DrxFOeRFnDu8ubzFse8Mr_s7E/) 或[Forum Post](https://forums.unrealengine.com/t/multithreading-and-performance-in-unreal/1216417/1)中阅读。

## 🎯 扩展 Unreal Editor

<table><tbody><tr><td>这一部分并非与 ChatGPT 一同编写。</td></tr></tbody></table>

您可以通过这个 github 仓库找到编辑图标，由 [github repo, made by EpicKiwi](https://github.com/EpicKiwi/unreal-engine-editor-icons) 制作。
...
*   运行时错误 - 在程序执行过程中发生的错误，导致程序崩溃或行为异常。如果你能够访问崩溃报告器，运行时错误可以相当快速地解决，崩溃报告器通常包含[调用栈](https://en.wikipedia.org/wiki/Call_stack)（可以精确指出导致崩溃的函数或代码）。

*   语义错误 - 当代码在语法上正确但行为不符合预期或预期时发生的逻辑错误。语义错误可能是最难解决的一种错误，因为你需要从逻辑而非语法上理解代码。


编译器只会对语法和链接器问题报错。编译错误是指[编译器](https://en.wikipedia.org/wiki/Compiler)无法编译的状态。这可能是由于代码中的错误，或者更不寻常的是，由于编译器本身的错误。

运行时错误可以通过[崩溃报告器](https://en.wikipedia.org/wiki/Crash_reporter)解决。而语义错误可以通过理解代码的逻辑推理来解决。

<Callout type="success">
  如果你感到卡壳或思维混乱，不妨花几分钟甚至几小时做点别的事情。比如出去走走、玩电子游戏、听音乐或看电影或视频。这可以帮助你大脑重新思考并更快地解决问题。
</Callout>

你可以在[Microsoft website](https://learn.microsoft.com/en-us/cpp/error-messages/compiler-errors-1/compiler-errors-c2000-c3999)上找到所有的编译器错误。
...
错误消息的[Link](https://learn.microsoft.com/en-us/cpp/error-messages/compiler-errors-1/compiler-error-c2007)。
...
错误消息[Link](https://learn.microsoft.com/en-us/cpp/error-messages/compiler-errors-1/compiler-error-c2065)。
...
错误消息的[Link](https://learn.microsoft.com/en-us/cpp/error-messages/compiler-errors-2/compiler-error-c2628)。
...
[作用域](<https://en.wikipedia.org/wiki/Scope_(computer_science)>)指的是代码中变量可访问的区域。在 C++中，作用域由 \{ 和 \} （大括号）定义。
...
有一个名为[《最佳实践（2019-2021）》的视频，由 Stephen Maloney 制作](https://www.youtube.com/watch?v=g7WVBZZTRDk)，介绍了其中一些最佳实践。

视频中还有一个[Google documentation](https://docs.google.com/document/d/1kIgOM7VONlPtx3WPiKdNVRYquX-GTduqSw0mU7on5h8)（如果视频内容不够），提供了更多关于他一些技巧和窍门的详细信息。
...
您可以通过[K2Node by Oscar Olsson](https://olssondev.github.io/2023-02-13-K2Nodes/)了解更多关于 K2Node 的信息。
...
你可以在[Unreal's docs](https://docs.unrealengine.com/5.3/en-US/core-redirects-in-unreal-engine/)中了解更多信息。

* * *

`MatchSubstring` 参数可用于任何核心重定向类型。如果存在且设置为 `true`，则 `OldName` 和 `NewName` 字段将被视为子字符串，而不是要求完全匹配。这使得单个核心重定向可以进行多次匹配。在下面的示例中，我们将从一个结构体和一个类开始。
...
你可以在 [curves on Unreal's docs](https://docs.unrealengine.com/5.3/en-US/API/Runtime/Engine/Curves/UCurveFloat/) 中了解更多关于曲线的信息。
