...
<ObsidianCallout type="IMPORTANT">
  <ObsidianCalloutTitle>
    如果你想在 Unreal Engine 中使用其他语言，比如 C#，那么强烈推荐你查看 Oscar Olsson 的这个仓库。
  </ObsidianCalloutTitle>
...
<ObsidianCallout type="NOTE">
  <ObsidianCalloutTitle>
    这个仓库是和 ChatGPT 一起创建的，用于协助写作和构思每一句话。虽然它提供了有价值的信息，但可能并不完全准确。如果你发现任何错误或虚假陈述，请随时创建一个新问题来报告它们，以便进一步改进和澄清。
  </ObsidianCalloutTitle>

  <ObsidianCalloutBody>
    你也可以发送一个新的 pull request 来对代码库进行正确的修改。
...
![jbtronics - CheatSheet Poster](https://github.com/MrRobinOfficial/Guide-UnrealEngine/raw/main/static/img/CheatSheet_Poster-1.png)
[jbtronics - 技巧海报](https://github.com/jbtronics/UE4-CheatSheet/blob/master/CheatSheet_Poster.pdf)

![Winslow - Unreal Engine 5 Blueprint CheatSheet Dark Theme](https://github.com/MrRobinOfficial/Guide-UnrealEngine/raw/main/static/img/unreal-engine-5-blueprint-cheat-sheet-dark-theme-1.png)
[Winslow - Unreal Engine 5 蓝图技巧海报 暗色主题](https://uecasts.com/resources/unreal-engine-5-blueprint-cheat-sheet-dark-theme?utm_source=epicgames\&utm_campaign=cheat_sheet_ue5\&utm_content=blueprint_dark)

![Winslow - Unreal Engine 5 C++ CheatSheet Dark Theme](https://github.com/MrRobinOfficial/Guide-UnrealEngine/raw/main/static/img/unreal-engine-5-c-plus-plus-cheat-sheet-dark-theme-1.png)[Winslow - Unreal Engine 5 C++ 技巧海报 暗色主题](https://uecasts.com/resources/unreal-engine-5-c-plus-plus-cheat-sheet-dark-theme?utm_source=epicgames\&utm_campaign=cheat_sheet_ue5\&utm_content=c_plus_plus_dark)

![Winslow - Unreal Engine 5 Editor CheatSheet Dark Theme](https://github.com/MrRobinOfficial/Guide-UnrealEngine/raw/main/static/img/unreal-engine-5-editor-cheat-sheet-dark-theme-1.png)[Winslow - Unreal Engine 5 编辑器暗黑主题作弊表](https://uecasts.com/resources/unreal-engine-5-editor-hotkeys-cheat-sheet-dark-theme?utm_source=epicgames\&utm_campaign=cheat_sheet_ue5\&utm_content=hotkeys_dark)

![VictoriaLyons - ProfilingCheatSheet](https://github.com/MrRobinOfficial/Guide-UnrealEngine/raw/main/static/img/ProfilingCheatSheet-1.png)[VictoriaLyons - 性能分析作弊表](https://www.reddit.com/r/unrealengine/comments/gqi2xu/quick_performance_cheat_sheet/)
...
* 成就系统（轻松在任何 UE4 项目中定义和管理成就）。
* 动画（使用矢量曲线动画化任何事物）。
* 附件同步（使任何武器都能拥有附件）。
* 音频计量轻量版（让你分析和计量子混音）。
* 自动大小评论（使评论框自动调整大小以适应内部节点）。
* 可蓝图化开发者设置（通过蓝图和编辑器访问配置文件，类似于 `UDeveloperSettings` ）。
* 常用地图（通过将它们存储在列表中，允许您快速切换到您正在处理的项目中经常使用的地图）。
* 自定义详细信息部分（通过将您的变量分组到自定义部分来组织详细信息面板）。
* DzX 径向菜单（径向菜单插件为饼图样式的组件设计提供了完全的自由度）。
* 增强输入序列（允许您处理输入动作及其序列）。
* 扩展调试库（扩展了部分原生调试功能，暴露了更多功能）。
* 文件系统库（在平静中打开、保存、复制、移动和删除文件和目录）。
* 游戏流程（允许用户创建状态机并配置它们用于各种目的）。
* 渐变生成器（帮助您创建复杂的渐变）。
* 实时蓝图调试器（将蓝图调试器的变量窗口直接集成到关卡编辑器的详细信息面板中，用于任何选定的蓝图类实例）。
* 大纲快捷方式（允许将快捷方式绑定到一些基本的世界大纲功能）。
* 项目声学（用于为您的 3D 环境添加精确声学的音频波形声学引擎）。
* 项目清理器（删除项目中的未使用资源和空文件夹）。
* 任务编辑器插件（管理和创建 NPC 的任务和对话，具有非常动态和可定制的功能）。
* 快速操作（在 Unreal 中快速查找任何内容）。
* 正则表达式在蓝图（蓝图可用正则表达式）。
* 重启虚幻编辑器（点击或使用快捷键重启虚幻编辑器的工具）。
* 屏幕淡入淡出库（插件，使在蓝图和 C++中实现屏幕淡入淡出变得简单）。
* SweejTech 音频检查器（允许你在紧凑的编辑器选项卡窗口中查看游戏中所有活动声音的详细有用信息）。
* 时间时钟（轻松追踪你在项目中需要工作多长时间，并使用闹钟管理你的时间）。
* 通用相机（为你的游戏设计的灵活相机）。
...
* 角色锁定器（在编辑关卡时可以锁定角色）。
* 高级运动系统重构（C++版本）。
* Agora 静态网格缩略图渲染器（在内容浏览器中提供一些关于静态网格的信息）。
* Aliaser（允许在内容浏览器中显示别名）。
* ALS-Refactored XT（扩展 ALS 重构，包含扩展的角色状态、改进的足迹系统、滑行、跳跃和攀墙功能）。
* AsyncLoadingScreen (能够添加自定义加载屏幕)。
* Audio Analysis Tools (提供多种分析音频数据的功能。与 Runtime Audio Importer 插件协同工作)。
* Audio Meters for Unreal Engine 5.1 (能够监控您的音频电平)。
* Batch Rename Tool (使用命名规则快速重命名多个资源)。
* BlueprintUE C++ (允许您将蓝图发送到关联到您的账户)。
* BUI Cheat Manager (作弊管理器，提供定义作弊快捷方式的方法)。
* 循环依赖检测器。
* CrystalNodes (美化虚幻编辑器图节点)。
* CsToCpp (C#到 UE C++编译器)。
* CustomShapeButton (允许在 UE5.3 中制作任意形状的按钮)。
* Elementus Inventory (数据驱动型库存与物品系统)。
* ExtendedUnreal (为蓝图提供实用函数)。
* FlatNodes (Unreal Engine 图节点的一种极简风格)。
* FlowGraph (为游戏流程设计的无设计依赖节点系统)。
* Function Picker (允许在 UE5.3 中的任何蓝图属性中直接选择函数)。
* Hotbar and Inventory System (易于使用的 Hotbar 和背包系统)。
* MDMetaDataEditor (能够编辑蓝图属性、函数以及函数和事件参数的元数据)。
* Neat Metadata (允许在蓝图中使用各种元数据，这些元数据通常仅在 C++中可用)。
* ObjectPool (通过从固定池中重复使用对象而不是单独分配和释放它们来提高性能和内存使用)。
* PBCharacterMovement (C++中的 HL2 风格经典 FPS 移动，适用于 UE)。
* 池管理器（帮助在 UE5.3 中重用对象）。
* 刷新所有节点（能够在单个蓝图实例中刷新所有节点。当蓝图版本未更新到其对应的 C++版本时很有用）。
* 串行通信（串行通信端口库）。
* 设置小部件构造器（从 UE5.3 中的数据表自动生成游戏设置 UI）。
* 子系统浏览器插件（添加子系统浏览器面板，用于探索运行中的子系统并编辑其属性）。
* 标签恢复（恢复已关闭的标签）。
* UE4/UMG 的 UI 缓动库。
* 虚幻引擎蓝图 CSV 解析。
* UnrealImGui (将 Dear ImGui 框架集成到 Unreal Engine 中)。
* VoxelCore (在 UE 中使用体素)。

### Epic Games 插件

* 由 Epic Games 制作的 Advanced Renamer。
* 游戏输入由 Epic Games 制作。
* 混沌载具由 Epic Games 制作（载具系统）。
* 通用 UI 由 Epic Games 制作。
* 增强输入由 Epic Games 制作。
* 由 Epic Games 制作的太阳位置计算器（实时天空）。

### MrRobinOfficial 的插件

* 通用时间（提供编辑 DateTime 和 Timespan 值的编辑器视图）。
* 通用车辆（扩展 Chaos 车辆系统）。
* IniParser (导入/导出 .ini 文件).

我计划制作更多插件，你可以在 Github 上找到我。
...
* Ctrl + Shift + Comma : GPU 可视化

## ⌛ 开始学习 C++

<table><tbody><tr><td>本节内容与 ChatGPT 共同编写。</td></tr></tbody></table>

强烈建议参加一个原版 C++的短期课程。这里有一个来自 Mosh 的~1 小时长的视频教程链接。

你也可以观看 GGameDev 关于开始学习 Unreal Engine C++的播放列表。

C++是一种静态类型、编译型、通用的编程语言，它结合了高级和低级特性。它由 Bjarne Stroustrup 于 1979 年在贝尔实验室开发，作为 C 语言的增强版，最初名为 C with Classes，后于 1983 年更名为 C++。

您可以在 Microsoft Learn 上阅读更多关于 C++语言参考的内容。
...
* Visual Studio：用于 Unreal Engine 开发的 Visual Studio IDE。它提供了一套强大的 C++工具，并与 Unreal Engine 无缝集成，提供了一个稳固的开发环境。 `Free` .

* Visual Studio Code (VSCode): Visual Studio Code 是一款轻量级、跨平台的代码编辑器，拥有丰富的扩展生态系统，包括用于 Unreal Engine 开发的扩展。 `Free` .

* Rider: Rider 是由 JetBrains 开发的流行 IDE，专为游戏开发设计，并提供了与 Unreal Engine 项目的良好集成。 `Cost` .
...
* Visual Assist：一款用于重构、阅读、编写、导航和生成 C/C++/C#代码的生产力工具。 `Cost` 和 `VS` 。

* UnrealMacroGenerator：提供宏编辑器，用于 Unreal Engine 的 Unreal C++。您可以创建宏并编辑已编写的宏。 `Free` 和 `VS` 。
...
* 垃圾回收和内存管理：虽然 C++要求手动内存管理，但 Unreal Engine 提供了一个垃圾回收器，可以高效地从内存中清除 `UObject` 类。通过手动内存处理的控制，你可以精确地指定何时分配和释放内存。
...
如果你希望扩展你对 C++和一般编程原则及理念的了解，我强烈推荐你阅读这一部分。
...
你可以在他们的文档中了解更多关于虚幻架构的信息。

关于虚幻架构的更多信息，我强烈推荐你可以观看关于虚幻引擎架构的视频。

你也可以观看 Kekdot 关于虚幻引擎多人框架的视频讨论。

<ObsidianCallout type="NOTE">
  <ObsidianCalloutTitle>
    这个架构基于多人游戏设置。但是，如果你在制作单人游戏，那么你可以忽略一些类。主要是 `APlayerState` 和 `AGameState` （以及它们的基类）。
  </ObsidianCalloutTitle>

  <ObsidianCalloutBody />
</ObsidianCallout>

你也可以观看 Alex Forsythe 的《虚幻引擎游戏框架：从 int main()到 BeginPlay》，他讲述了虚幻引擎如何从源代码启动你的游戏/编辑器，以及这些类如何相互协作。

## ⚓ 指南

你可以找到更多关于虚幻引擎指南的信息，包括在编辑器内部（资源创建），以及它们的编码标准。

### 🎳 命名规范

<table><tbody><tr><td>本节内容由 ChatGPT 共同编写。</td></tr></tbody></table>

有一个 GitHub 仓库，讨论了虚幻引擎的命名规范。该仓库非常详细，解释了应该如何命名你的资源以及代码。仓库名为 Michael Allar 的《虚幻引擎风格指南》。
...
<table><tbody><tr><td>通用语言特性 arg - 参数 arr - 数组 async - 异步 attr - 属性 auth - 认证 btn - 按钮 buff - 缓冲区 ctx - 上下文 const - 常量 db - 数据库 dest - 目的地 desc - 描述 doc 或 docs - 文档 dir - 方向或目录（视上下文而定） elem - 元素 err - 错误 e 或 evt - 事件 exe - 执行 expr - 表达式 ext - 扩展 func - 功能 fmt - 格式 gen - 生成 hex - 十六进制 impl - 实现 imp - 导入 i 或 idx - 索引 info - 信息 init - 初始化 it 或 iter - 迭代器 ident - 标识符 lang - 语言 len - 长度 lvl - 等级 lib - 图书馆 loc - 位置 msg - 消息 num - 数字 obj - 对象 opt - 选项 out - 输出 pkg - 包 param - 参数 px - 像素 pos - 位置 prev - 上一个 priv - 私有的 pub - 公共 q - 查询 rand - 随机 rng - 范围 ref - 参考 rm 或 rmv - 移除 req - 请求 res - 结果或响应（视上下文而定） ret - 返回 sel - 选择 sep - 分隔符 sec - 序列 sol - 求解器 src - 源 spec - 指定符或规范（视上下文而定） std - 标准 stdio - 标准输入输出 stmt - 声明 stat - 统计 str - 字符串 sync - 同步 tmp - 温度 temp - 暂时 util - 实用 val - 值 var - 变量 ws - 空格 win - Windows wiz - 巫师 虚幻引擎特性 PC - 表示一个变量是 PlayerController LP - 表示一个变量是 LocalPlayer Char - 表示该变量是一个字符（不要与 char 数据类型混淆） Comp - 表示该变量是一个组件 Ptr - 表示该变量是指向一个对象的指针。 Ref - 表示该变量是指向一个对象的引用。 dt - Delta Time Networking OAuth or Open Authentication – 一个开放标准，用于认证应用程序或网站访问内容。 TCP or Transmission Control Protocol – 一个标准，定义了系统之间如何交换消息。 UDP 或用户数据报协议——定义系统间交换消息的标准。 工具/框架 IDE 或集成开发环境——为程序员提供软件开发功能的软件应用程序。 JSON 或 JavaScript 对象表示法——一种使用 JavaScript 2 记法编写的文件格式，广泛用于网络数据传输。 XML 或可扩展标记语言——一种主要用于存储和传输数据的标记语言。 SQL 或结构化查询语言——一种用于数据存储、检索和修改的查询语言。 CSV 或逗号分隔值——CSV 文件是一种使用逗号分隔值的分隔文本文件。 数学 add - 加法 sub - 减法 mul - 乘法 div - 除法 abs - 绝对 sin - 正弦 cos - 余弦 tan - 正切 rad - 弧度 r - 半径 frac - 分数 freq - 频率 long - 经度或纵坐标（视上下文而定） lat - 纬度或横坐标（视上下文而定） sqrt - 平方根 mod - 模数 min - 最小值 max - 最大值 lerp - 线性插值 杂项 API 或应用程序编程接口——用于连接多个隔离组件的接口。 SDK 或软件开发工具包——一组通常在特定平台上开发时所需的软件。 TDD 或测试驱动开发——TDD 是一种基于短开发周期重复的软件开发过程：需求被转化为具体的测试用例，然后修复代码以确保测试通过。 UUID 或通用唯一识别码——UUID 是一种 128 位的数字，用于在计算机系统中识别信息。 GUI 或图形用户界面 - 图形用户界面（GUI）是一种允许用户通过图形界面与电子设备交互的用户界面形式。 misc - 其他 os - 操作系统 org - 组织 pwr - 电源 pref - 偏好 repo - 仓库</td></tr></tbody></table>
...
<ObsidianCallout type="TIP">
  <ObsidianCalloutTitle>
    你知道吗， `F` 前缀实际上代表 `Float` （浮点数）。但它意外地被广泛传播，并失去了原本的含义。
  </ObsidianCalloutTitle>
...
你可以通过 BenUi 了解更多关于 UPROPERTY 的信息。
...
您可以阅读更多关于 UPROPERTY 的信息，由 BenUi 提供。
...
你可以通过 BenUi 了解更多关于 USTRUCT 的信息。
...
你可以在 Unreal 的文档中了解更多关于 TCHAR 的信息。
...
<table><tbody><tr><td>char ， signed char 和 unsigned char 至少为 8 位 signed short ， unsigned short ， signed int 和 unsigned int 至少为 16 位 signed long 和 unsigned long 至少为 32 位 signed long long 和 unsigned long long 至少为 64 位</td></tr></tbody></table>

你可以在 Stack Overflow 上从 Alex 那里了解更多详细信息。
...
在虚幻引擎中，您现在可以用 `int64` 代替 `signed long long` 来表示 64 位整数。这些别名称为 typedef，您可以在 C++文档中了解更多关于 typedef 关键字的信息。
...
<ObsidianCallout type="WARNING">
  <ObsidianCalloutTitle>
    `uint16` 、 `uint32` 、 `uint64` 、 `int8` 、 `int16` 和 `double` 在 UHT [3](#user-content-fn-2) 中不受支持。这意味着无法将其暴露给蓝图。
  </ObsidianCalloutTitle>
...
您可以查阅文档了解更多关于字符串处理的信息。

### 文本宏

<table><tbody><tr><td>TEXT - 此宏用于指定宽字符（UTF-16）编码。这使得字符串字面量与平台无关。如果没有此宏，你将使用 ANSI 编码（这可能导致在其他机器上出现问题）。 INVTEXT - 此宏调用 FText::AsCultureInvariant(TEXT(InTextLiteral))，并将 InTextLiteral 作为参数。有助于从给定的字符串字面量创建文化不变的 FText。 LOCTEXT - 此宏用于为本地化创建 FText 字面量。它接受一个命名空间和一个键来标识本地化字符串。</td></tr></tbody></table>
...
<ObsidianCallout type="NOTE">
  <ObsidianCalloutTitle>
    默认情况下，Unreal 在执行此格式化时会使用本地文化。如果你希望禁用文化格式化，请使用 `FText::AsCultureInvariant` 函数。
  </ObsidianCalloutTitle>
...
<ObsidianCallout type="NOTE">
  <ObsidianCalloutTitle>
    在虚幻引擎 5.0 及以上版本中，默认情况下所有与数学相关的数据类型都使用 `double` 作为后端数据类型。这使虚幻引擎能够支持大型世界坐标（LWC）。
  </ObsidianCalloutTitle>
...
<ObsidianCallout type="NOTE">
  <ObsidianCalloutTitle>
    使用 `FVector4f` 代表 `float` ，使用 `FVector4d` 代表 `double` ，作为后端转换的明确数据类型。
  </ObsidianCalloutTitle>
...
<ObsidianCallout type="NOTE">
  <ObsidianCalloutTitle>
    使用 `FVector3f` 作为 `float` ，使用 `FVector3d` 作为 `double` ，作为后端转换的显式数据类型。
  </ObsidianCalloutTitle>
...
<ObsidianCallout type="NOTE">
  <ObsidianCalloutTitle>
    使用 `FVector2f` 表示 `float` ，使用 `FVector2d` 表示 `double` ，作为后端转换的显式数据类型。
  </ObsidianCalloutTitle>
...
<ObsidianCallout type="NOTE">
  <ObsidianCalloutTitle>
    使用 `FInt32Point` 代表 `int32` ， `FUint32Point` 代表 `uint32` ， `FInt64Point` 代表 `int64` ，以及 `FUint64Point` 代表 `uint64` ，作为后端转换的明确数据类型。
  </ObsidianCalloutTitle>
...
<ObsidianCallout type="NOTE">
  <ObsidianCalloutTitle>
    使用 `FInt32Rect` 表示 `int32` ， `FUint32Rect` 表示 `uint32` ， `FInt64Rect` 表示 `int64` ，以及 `FUint64Rect` 表示 `uint64` ，作为后端转换的显式数据类型。
  </ObsidianCalloutTitle>
...
<ObsidianCallout type="NOTE">
  <ObsidianCalloutTitle>
    使用 `FRotator3f` 表示 `float` ，使用 `FRotator3d` 表示 `double` ，作为后端转换的明确数据类型。
  </ObsidianCalloutTitle>
...
<ObsidianCallout type="NOTE">
  <ObsidianCalloutTitle>
    使用 `FQuat4f` 代替 `float` ，使用 `FQuat4d` 代替 `double` ，作为后端转换的明确数据类型。
  </ObsidianCalloutTitle>
...
<ObsidianCallout type="NOTE">
  <ObsidianCalloutTitle>
    使用 `FTransform3f` 代表 `float` ，使用 `FTransform3d` 代表 `double` ，作为后端转换的明确数据类型。
  </ObsidianCalloutTitle>
...
<ObsidianCallout type="NOTE">
  <ObsidianCalloutTitle>
    将 `FPlane4f` 用于 `float` ，将 `FPlane4d` 用于 `double` ，作为后端转换的明确数据类型。
  </ObsidianCalloutTitle>
...
<ObsidianCallout type="NOTE">
  <ObsidianCalloutTitle>
    在后端转换时，使用 `FMatrix44f` 作为 `float` ，使用 `FMatrix44d` 作为 `double` ，作为显式的数据类型。
  </ObsidianCalloutTitle>
...
<ObsidianCallout type="NOTE">
  <ObsidianCalloutTitle>
    使用 `FSphere3f` 表示 `float` ，使用 `FSphere3d` 表示 `double` ，作为后端转换的明确数据类型。
  </ObsidianCalloutTitle>
...
<ObsidianCallout type="NOTE">
  <ObsidianCalloutTitle>
    使用 `FBox3f` 表示 `float` ，使用 `FBox3d` 表示 `double` ，作为后端转换的明确数据类型。
  </ObsidianCalloutTitle>
...
<ObsidianCallout type="NOTE">
  <ObsidianCalloutTitle>
    使用 `FBox2f` 表示 `float` ，使用 `FBox2d` 表示 `double` ，作为后端转换的明确数据类型。
  </ObsidianCalloutTitle>
...
<ObsidianCallout type="NOTE">
  <ObsidianCalloutTitle>
    将 `FRay3f` 用于 `float` ，将 `FRay3d` 用于 `double` ，作为后端转换的显式数据类型。
  </ObsidianCalloutTitle>
...
![Collections](https://github.com/MrRobinOfficial/Guide-UnrealEngine/raw/main/static/img/Collections.png)
...
<ObsidianCallout type="WARNING">
  <ObsidianCalloutTitle>
    如果你试图使用 `TInlineAllocator` 在栈上分配堆对象，Unreal 会默认采用堆分配。
  </ObsidianCalloutTitle>
...
<ObsidianCallout type="WARNING">
  <ObsidianCalloutTitle>
    如果你添加的元素数量超过已分配的容量，Unreal 会默认采用堆分配。
  </ObsidianCalloutTitle>
...
<ObsidianCallout type="NOTE">
  <ObsidianCalloutTitle>
    Unreal 会将栈分配的数组视为不同的数据类型，与普通数组不同。为了适应这一点，请使用 `TArrayView` 。
  </ObsidianCalloutTitle>
...
<ObsidianCallout type="WARNING">
  <ObsidianCalloutTitle>
    Unreal 不支持 `TMultiMap` 与 UHT [3](#user-content-fn-2) 。这意味着你不能将其暴露给蓝图。
  </ObsidianCalloutTitle>
...
<ObsidianCallout type="WARNING">
  <ObsidianCalloutTitle>
    Unreal Engine 不支持 `TStaticArray` 与 UHT [3](#user-content-fn-2) 。这意味着你不能将其暴露给蓝图。要在蓝图中使用静态数组，请在 `TArray` 属性上使用 `FixedSized` 指定符。
  </ObsidianCalloutTitle>
...
<ObsidianCallout type="NOTE">
  <ObsidianCalloutTitle>
    `TList` 不为每个节点提供插入或删除功能。如果您希望使用这些功能，请使用 `TLinkedList` 。
  </ObsidianCalloutTitle>
...
<ObsidianCallout type="NOTE">
  <ObsidianCalloutTitle>
    一般来说，您几乎总是应该使用 `TArray` ，除非您有使用链表的具体原因。
  </ObsidianCalloutTitle>
...
<ObsidianCallout type="WARNING">
  <ObsidianCalloutTitle>
    `TArrayView` 是一个固定大小且独立的数组。这意味着它不会影响其原始分配，也不支持 Add()或 Remove()函数。
  </ObsidianCalloutTitle>
...
<ObsidianCallout type="NOTE">
  <ObsidianCalloutTitle>
    仍然可以使用 Algo 库，它提供了用于 TArrayView 和 TArray 的函数，例如 Algo::Reverse() 和 Algo::ForEach()。
  </ObsidianCalloutTitle>

  <ObsidianCalloutBody />
</ObsidianCallout>

<ObsidianCallout type="CAUTION">
  <ObsidianCalloutTitle>
    避免将 `TArrayView` 与临时数组变量一起使用。因为数组可能会超出作用域，这将使 `TArrayView` 变量无效并指向不同的内存块。这是因为视图依赖于数组的内存块。
  </ObsidianCalloutTitle>
...
<ObsidianCallout type="NOTE">
  <ObsidianCalloutTitle>
    `UPROPERTY` 支持常规枚举，并取代了使用 `TEnumAsByte` 的需求。
  </ObsidianCalloutTitle>
...
> \[!警告] 对原始指针使用名为 `IsValid()` 的函数，并对其进行任何操作。
...
<ObsidianCallout type="NOTE">
  <ObsidianCalloutTitle>
    `ActorPtr` 使用 `UPROPERTY()` 标记，以通知 UHT [3](#user-content-fn-2) 该指针存在。当指针未被使用时，垃圾收集器会将其标记并删除其内存。请注意，此过程可能需要几个帧，并非即时完成。因此，请始终使用 `IsValid()` 函数，该函数还会检查指针是否已被标记用于垃圾收集。避免使用手动检查，例如 `PlayerCharacter != nullptr` （因为它与垃圾收集系统不兼容）。
  </ObsidianCalloutTitle>
...
<ObsidianCallout type="WARNING">
  <ObsidianCalloutTitle>
    如果其他内容正在引用 `ActorPtr` ，该指针将不会通过垃圾收集被销毁（除非它是弱指针）。
  </ObsidianCalloutTitle>
...
![TSoftObjectPtr](https://github.com/MrRobinOfficial/Guide-UnrealEngine/raw/main/static/img/TSoftObjectPtr_BP.png)
...
<ObsidianCallout type="WARNING">
  <ObsidianCalloutTitle>
    不要使用 `FSoftObjectPath` 或 `FSoftObjectPtr` 。用于内部用途。
  </ObsidianCalloutTitle>
...
![TSoftClassPtr](https://github.com/MrRobinOfficial/Guide-UnrealEngine/raw/main/static/img/TSoftClassPtr_BP.png)
...
<ObsidianCallout type="NOTE">
  <ObsidianCalloutTitle>
    如果你希望在编译时获得一个"真正"的常量值，并且不期望编译器有任何处理，你可以使用 `consteval` 关键字。这会强制编译器在编译时编译你的代码。
  </ObsidianCalloutTitle>
...
<ObsidianCallout type="WARNING">
  <ObsidianCalloutTitle>
    `consteval` 和 `constinit` 关键字仅在 C++ 20 版本中受支持。这意味着如果你的编译器不支持这些关键字，你将无法使用它们。
  </ObsidianCalloutTitle>
...
> \[!警告] 在 Unreal Engine 中，建议使用 `NewObject()` 和 `MakeShared()` 这类内置的内存管理函数来为对象分配内存，而不是使用 `new` 和 `delete` 。使用 `new` 和 `delete` 可能会干扰垃圾回收器，导致游戏出现内存泄漏或崩溃。
...
<ObsidianCallout type="NOTE">
  <ObsidianCalloutTitle>
    在虚幻引擎中，更常见的是使用名为 `FORCEINLINE` 的宏，它本质上扩展为 `force_inline` 关键字。
  </ObsidianCalloutTitle>
...
<ObsidianCallout type="CAUTION">
  <ObsidianCalloutTitle>
    不要在 Unreal Engine 项目中使用 C++版本的类型转换。相反，应使用引擎提供的 `Cast()` 。这种转换针对 UE 进行了优化。
  </ObsidianCalloutTitle>
...
<ObsidianCallout type="NOTE">
  <ObsidianCalloutTitle>
    如果您希望在 Unreal 中使用 `static_cast()` ，则应使用引擎提供的 `StaticCast()` 。这将生成相同的代码，但会修复 Visual Studio 中的一些问题。
  </ObsidianCalloutTitle>
...
![Lifecycle breakdown](https://github.com/MrRobinOfficial/Guide-UnrealEngine/raw/main/static/img/ActorLifeCycle.png)
...
<ObsidianCallout type="WARNING">
  <ObsidianCalloutTitle>
    `BlueprintNativeEvent` 允许你在 C++ 或蓝图中对函数进行重写，但如果你使用 `BlueprintImplementableEvent` ，它只允许在蓝图中对函数进行重写。
  </ObsidianCalloutTitle>
...
<ObsidianCallout type="NOTE">
  <ObsidianCalloutTitle>
    你是从接口本身调用接口函数。也就是说，不能使用 `ACarPawn->Explode()` （接口函数），而必须使用 `IVehicle::Execute_Explode()` ，并将参数设置为接口函数要调用的对象。
  </ObsidianCalloutTitle>
...
> \[!警告] `Cast<>` 方法只有在接口在 C++类中实现时才有效。如果接口是在蓝图（Blueprint）中实现的，那么强制类型转换将返回 `nullptr` 。你可以使用 `TScriptInterface<>` 来安全地复制接口指针。或者，你可以使用弱接口指针（ `TWeakInterfacePtr` ）。
...
<ObsidianCallout type="WARNING">
  <ObsidianCalloutTitle>
    为了让垃圾回收器完成其工作（确定哪些对象可以安全删除，对于容器而言），它必须遍历每个对象的每个字段。虽然 Unreal 提供了多种类型的容器（ `TArray` ， `TMap` ， …），但垃圾回收器只考虑 `TArray` 中的指针。
  </ObsidianCalloutTitle>
...
<ObsidianCallout type="NOTE">
  <ObsidianCalloutTitle>
    使用 `UPROPERTY` 声明的原始指针将被设置为 `nullptr`
  </ObsidianCalloutTitle>
...
![Soft vs hard references](https://github.com/MrRobinOfficial/Guide-UnrealEngine/raw/main/static/img/Soft_Hard_Refs.png)
...
<ObsidianCallout type="NOTE">
  <ObsidianCalloutTitle>
    `StaticCast()` 函数只是转发 C++中的内置转换操作，该操作称为 `static_cast()` 。这意味着在 Unreal Engine `UObject` 中没有安全性保证。
  </ObsidianCalloutTitle>
...
<table><tbody><tr><td>预定义的日志类别 LogPath LogController LogPhysics LogBlueprint LogBlueprintUserMessages LogAnimation LogRootMotion LogLevel LogSkeletalMesh LogStaticMesh LogNet LogRep LogNetPlayerMovement LogNetTraffic LogRepTraffic LogNetFastTArray LogNetDormancy LogSkeletalControl LogSubtitle LogTexture LogPlayerManagement LogSecurity LogEngineSessionManager LogHAL LogSerialization LogUnrealMath LogUnrealMatrix LogContentComparisonCommandlet LogNetPackageMap LogNetSerialization LogMemory LogProfilingDebugging LogCore LogOutputDevice LogSHA LogStats LogStreaming LogInit LogExit LogExec LogScript LogLocalization LogLongPackageNames LogProcess LogLoad LogTemp</td></tr></tbody></table>
...
<ObsidianCallout type="NOTE">
  <ObsidianCalloutTitle>
    FText 不支持 `UE_LOGFMT()` ，要使用 `FText` ，需要通过调用 `ToString()` 函数将其转换为 `FString` 。
  </ObsidianCalloutTitle>
...
![Assertions](https://github.com/MrRobinOfficial/Guide-UnrealEngine/raw/main/static/img/Assertions.png)
...
![Delegates](https://github.com/MrRobinOfficial/Guide-UnrealEngine/raw/main/static/img/Delegates.png)

代表者本质上是一种类型安全的函数指针，可以用来将一个或多个函数绑定到事件上，然后在事件发生时触发这些函数。

在蓝图（Blueprint）中，你可能会遇到"事件调度器"，其底层实际上就是多播代表者。

Unreal 使用三种类型的代表者：单播、多播和动态。
...
![Draw Debug Point](https://github.com/MrRobinOfficial/Guide-UnrealEngine/raw/main/static/img/debugging/draw_point.png)
...
![Draw Debug Sphere](https://github.com/MrRobinOfficial/Guide-UnrealEngine/raw/main/static/img/debugging/draw_sphere.png)
...
![Draw Circle](https://github.com/MrRobinOfficial/Guide-UnrealEngine/raw/main/static/img/debugging/draw_circle.png)
...
![Draw Cricle Arc](https://github.com/MrRobinOfficial/Guide-UnrealEngine/raw/main/static/img/debugging/draw_circle_arc.png)
...
![Draw Debug 2D Donut](https://github.com/MrRobinOfficial/Guide-UnrealEngine/raw/main/static/img/debugging/draw_2d_donut.png)
...
![Draw Debug Soild Box](https://github.com/MrRobinOfficial/Guide-UnrealEngine/raw/main/static/img/debugging/draw_solid_box.png)
...
![Draw Debug Wired Box](https://github.com/MrRobinOfficial/Guide-UnrealEngine/raw/main/static/img/debugging/draw_wired_box.png)
...
![Draw Debug Cylinder](https://github.com/MrRobinOfficial/Guide-UnrealEngine/raw/main/static/img/debugging/draw_cylinder.png)
...
![Draw Debug Capsule](https://github.com/MrRobinOfficial/Guide-UnrealEngine/raw/main/static/img/debugging/draw_capsule.png)
...
![Draw Debug Cone](https://github.com/MrRobinOfficial/Guide-UnrealEngine/raw/main/static/img/debugging/draw_cone.png)
...
![Draw Debug Plane](https://github.com/MrRobinOfficial/Guide-UnrealEngine/raw/main/static/img/debugging/draw_plane.png)
...
![Draw Debug Line](https://github.com/MrRobinOfficial/Guide-UnrealEngine/raw/main/static/img/debugging/draw_line.png)
...
![Draw Debug Arrow](https://github.com/MrRobinOfficial/Guide-UnrealEngine/raw/main/static/img/debugging/draw_arrow.png)
...
![Draw Debug Crosshair](https://github.com/MrRobinOfficial/Guide-UnrealEngine/raw/main/static/img/debugging/draw_crosshair.png)
...
![Draw Debug Camera](https://github.com/MrRobinOfficial/Guide-UnrealEngine/raw/main/static/img/debugging/draw_camera.png)
...
![Draw Debug Mesh](https://github.com/MrRobinOfficial/Guide-UnrealEngine/raw/main/static/img/debugging/draw_mesh.png)
...
![Draw Debug String](https://github.com/MrRobinOfficial/Guide-UnrealEngine/raw/main/static/img/debugging/draw_string.png)
...
![Draw Debug Centripetal catmull-rom spline](https://github.com/MrRobinOfficial/Guide-UnrealEngine/raw/main/static/img/debugging/draw_centripetal_catmull_rom_spline.png)
...
<ObsidianCallout type="NOTE">
  <ObsidianCalloutTitle>
    如果计时器将要被调用的对象（例如 Actor）在时间结束前被销毁，计时器将自动取消。在这种情况下，计时器句柄将变得无效，函数将不会被调用。
  </ObsidianCalloutTitle>
...
<ObsidianCallout type="TIP">
  <ObsidianCalloutTitle>
    使用小于或等于零的速率调用 `SetTimer()` 与调用 `ClearTimer()` 相同。
  </ObsidianCalloutTitle>
...
<ObsidianCallout type="NOTE">
  <ObsidianCalloutTitle>
    为了避免同时使用 `UE_DECLARE_GAMEPLAY_TAG_EXTERN()` 和 `UE_DEFINE_GAMEPLAY_TAG` ，你可以改用 `UE_DEFINE_GAMEPLAY_TAG_STATIC()` 。但是，这个宏应该只在使用声明的实现文件中使用。
  </ObsidianCalloutTitle>
...
![Common Errors](https://github.com/MrRobinOfficial/Guide-UnrealEngine/raw/main/static/img/Cpp_Errors.png)
...
<ObsidianCallout type="WARNING">
  <ObsidianCalloutTitle>
    Typedefs 不适用于 UHT [8](#user-content-fn-1) 。这意味着你不能将其暴露给 Blueprint。
  </ObsidianCalloutTitle>
...
<ObsidianCallout type="WARNING">
  <ObsidianCalloutTitle>
    UHT [3](#user-content-fn-2) 不支持在 `USTRUCT` 内部暴露函数。然而，你仍然可以在 `USTRUCT` 内部有函数，但不仅暴露给蓝图。
  </ObsidianCalloutTitle>
...
![Math Node Example](https://github.com/MrRobinOfficial/Guide-UnrealEngine/raw/main/static/img/math_node_example.png)
...
<ObsidianCallout type="NOTE">
  <ObsidianCalloutTitle>
    在命令行界面（CLI）的上下文中，美元符号（ `$` ）通常被称为“提示符号”或简单地称为“提示”。它表示 CLI 已准备好接收用户的输入。提示的具体外观和行为可能会根据所使用的操作系统和外壳而有所不同。
  </ObsidianCalloutTitle>
...
![Original Values](https://github.com/MrRobinOfficial/Guide-UnrealEngine/raw/main/static/img/OriginalValues.jpg)
...
![NoCoreRedirect](https://github.com/MrRobinOfficial/Guide-UnrealEngine/raw/main/static/img/NoCoreRedirect.jpg)

代码中属性名称已更改，但未创建核心重定向。因此，没有数据值迁移到新属性。

![CoreRedirectWithoutMatchSubstring](https://github.com/MrRobinOfficial/Guide-UnrealEngine/raw/main/static/img/CoreRedirectWithoutMatchSubstring.jpg)

\`PropertyRedirects=(OldName="TestInt",NewName="TestInteger")\` 导致只有两个名称完全匹配的属性迁移其数据。

![CoreRedirectWithMatchSubstring](https://github.com/MrRobinOfficial/Guide-UnrealEngine/raw/main/static/img/CoreRedirectWithMatchSubstring.jpg)

\`PropertyRedirects=(OldName="TestInt",NewName="TestInteger",MatchSubstring=true)\` 由于子字符串匹配，导致我们所有的四个属性迁移。

<ObsidianCallout type="NOTE">
  <ObsidianCalloutTitle>
    由于 `MatchSubtring` 需要更彻底地检查传入的资产，因此可能会影响启动时间。 `MatchSubstring` 旨在作为进行大规模更改时的临时修复。建议立即重新保存涉及这些更改的资产，并将任何相关的代码更改与项目源代码数据库一起检查入，并删除核心重定向而不将其提交到源代码控制。
  </ObsidianCalloutTitle>
...
| 作者 | 标题 | 长度 | 链接 |
| --- | --- | --- | --- |
| Mosh Hamedani | C++ 初学者教程 - 1 小时内学会 C++ | 01:22:55 | YouTube |
| Alex Forsythe | 蓝图与 C++：它们如何协同工作以及为何你应该同时使用两者 | 47:13 | YouTube |
| Alex Forsythe | 虚幻引擎游戏框架：从 int main()到 BeginPlay | 27:22 | YouTube |
| Alex Forsythe | 虚幻引擎中的多人游戏：如何理解网络复制 | 22:07 | YouTube |
| 亚历克斯·福赛斯 | 当虚幻编辑器崩溃时你该怎么办？ | 13:04 | YouTube |
| 虚幻引擎 | UE5 中的遮罩和资产制作 | 34:07 | YouTube |
| 虚幻引擎 | 在虚幻引擎 5 中构建开放世界 | 49:41 | YouTube |
| 虚幻引擎 | 35 个你可能不知道的 UE5 功能 | 49:55 | YouTube |
| 阿米尔·安萨里 | 虚幻引擎过载 - 软硬引用 | 01:13:35 | YouTube |
| UNF 游戏 | 虚幻引擎 5 入门建模教程 - 学习在虚幻引擎中建模！ | 02:12:34 | YouTube |
...
| 作者                | 标题                   | 描述                     | 链接                                                                         |
| ----------------- | -------------------- | ---------------------- | -------------------------------------------------------------------------- |
| Naotsun           | UnrealMacroGenerator |                        | https://marketplace.visualstudio.com/items?itemName=Naotsun.Naotsun-UE-UMG |
| Sébastien Rancoud | blueprintUE          | 非官方工具，旨在帮助虚幻引擎开发者      | https://blueprintue.com/                                                   |
| 马特·戈德博尔特          | 编译器探索器               | 从你的网络浏览器交互式运行编译器并与汇编交互 | https://godbolt.org/                                                       |
|                   | 虚幻引擎 4 控制台变量和命令      | 所有 UE 命令列表             | https://digilander.libero.it/ZioYuri78/                                    |

### 杂项

| 作者       | 标题    | 描述              | 链接                                                                                         |
| -------- | ----- | --------------- | ------------------------------------------------------------------------------------------ |
| 托马斯·英格拉姆 | 开发者笔记 | 查看和发布开发者文档中的笔记。 | https://chrome.google.com/webstore/detail/developer-notes/fchdfdnnpkphopmdaochdfnmcahndmnb |
